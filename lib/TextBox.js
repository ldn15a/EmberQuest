...
function TextBox (text, options) {
    	...
    	if(tb.isShowing && tb.private.currentTextObject != undefined){
	    	...
	    	if(keyboard.getKeyStatus("e") == "pressed"){
	    		...
	    		if(tb.private.currentTextObject.hasOptions){
	    			...
	    			tb.selectedOption = tb.private.currentTextObject.optionsFull[tb.private.selectionIndex];
	    		}else{
	    			//I do this just in case.
	    			tb.selectedOption = tb.private.currentTextObject.fullString;
	    		}
	    		...
	    		if(tb.private.currentTextObject.hasOptions){
	    			...
	    			if(keyboard.getKeyStatus("w") == "pressed"){
	    				...
	    				if(tb.private.selectionIndex < 0){
	    					tb.private.selectionIndex = tb.private.currentTextObject.optionsDisplay.length-1;
	    				}
	    			...
	    			if(keyboard.getKeyStatus("s") == "pressed"){
	    				...
	    				if(tb.private.selectionIndex > tb.private.currentTextObject.optionsDisplay.length-1){
    ...
    tb.updateText = function(){
    	...
    	tb.private.currentDisplayText = tb.private.currentTextObject.displayString;
    	tb.changeMainText();
    	if(tb.private.currentTextObject.hasOptions){
    		tb.renderOptions();
    	}
	...
	tb.changeMainText = function () {
		tb.private.mainTextElement.innerHTML = tb.private.currentDisplayText;
	...
	tb.renderOptions = function () {
		...
		for (var i = 0; i < tb.private.currentTextObject.optionsDisplay.length; i++) {
			...
			lastOption.innerHTML = tb.private.currentTextObject.optionsDisplay[i];
