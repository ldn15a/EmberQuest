var HealthGraphic = function(x,y,mode){
	...
	healthGraphic.initialize = function(){
		...
		healthGraphic.asset = playerHeart;

		//healthGraphic.graphic = new PIXI.Graphics();
		
		healthGraphic.graphic = new PIXI.Sprite(PIXI.Loader.shared.resources[healthGraphic.asset.texture].texture);
		healthGraphic.graphic.position.set(x, y);
		healthGraphic.graphic.scale.x = healthGraphic.asset.scaleX;
		healthGraphic.graphic.scale.y = healthGraphic.asset.scaleY;
		healthGraphic.graphic.anchor.x = healthGraphic.asset.anchorX;
		healthGraphic.graphic.anchor.y = healthGraphic.asset.anchorY;
		healthGraphic.hitboxWidth = healthGraphic.asset.hbW * healthGraphic.graphic.width;
		healthGraphic.hitboxHeight = healthGraphic.asset.hbH * healthGraphic.graphic.height;
		//healthGraphic.graphic.position.set(x,y);
		...
		if(mode == 1){
			//width = 40;
			//height = 40;s
			healthGraphic.graphic.scale.x = 40/16*healthGraphic.asset.scaleX;
			healthGraphic.graphic.scale.y = 40/16*healthGraphic.asset.scaleY;
		}
		...
		if(healthGraphic.mode == 2){
			...
			healthGraphic.graphic.scale.x = 0.1*healthGraphic.asset.scaleX;
			healthGraphic.graphic.scale.y = 0.1*healthGraphic.asset.scaleY;
	...
	healthGraphic.update = function(){
		if(healthGraphic.mode == 2){
			...
			healthGraphic.graphic.scale.x = healthGraphic.t/healthGraphic.endTime*healthGraphic.asset.scaleX;
			healthGraphic.graphic.scale.y = healthGraphic.t/healthGraphic.endTime*healthGraphic.asset.scaleY;
