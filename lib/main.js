...
var controls = ["w", "a", "s", "d", " ", "j"];
var keyboard = Input (controls);
var testingTextBox = TextBox ();
...
function setup() {
	state = play;
  	...
  	for(var i = 0;i < 25;i++){
		...
		window.addEventListener("keydown", keyboard.record);
		window.addEventListener("keyup", keyboard.record);
		window.addEventListener("keypress", keyboard.record);
