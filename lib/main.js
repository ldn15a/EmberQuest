...
var unload = function () {
	...
	for (var i = 0; i < arrays.length; i++) {
		var minimumLength = 0;
		while (arrays[i].length > minimumLength){
			try {
				arrays [i].unload ();
			} catch (error) {
				console.error ("Unable to unload object from stage");
				minimumLength++;
			}
		}
	}
...
function setup() {
	...
	testingArea = DebugArea();
	testingArea.loadMapObj (function(){showMainMenu = true;});
