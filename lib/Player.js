var Player = function(x,y,asset){
	...
	player.wetnessControl = function(){
		...
		if(wetnessGraphic != undefined){
			wetnessGraphic.updateVisuals();
		}
	}
	...
	player.changeHealth = function(offset){
		...
		if(offset < 0){
			audio.forcePlay("loseLife");
		}else if(offset > 0){
			audio.forcePlay("recoveringHealth");
		}
	...
	player.load = function(){
		...
		player.updateHealthGraphics();
		player.wetnessGraphic = WetnessGraphic();
	}
