var Player = function(x,y,asset){
	...
	player.update = function(){
		...
		if(player.fireParticleTimer > 3){
			player.fireParticleArray.push(FireParticle(player.character.graphic.x,player.character.graphic.y,player));
	...
	player.unload = function(){
		player.destroyHealthGraphics();
		while(player.fireParticleArray.length > 0){
			player.fireParticleArray[0].destroy();
		}
	...
	player.scroll = function(){
		...
		for(var i = 0;i < player.fireParticleArray.length;i++){
			player.fireParticleArray[i].scroll();
		}
	}
	...
	player.depth = function(){
		...
		for(var i = 0;i < player.fireParticleArray.length;i++){
			player.fireParticleArray[i].depth();
		}
