var Player = function(x,y,asset){
	...
	player.update = function(){
		...
		player.updateSpriteBasedOnHealth();

		if(player.health > 0 && player.character.getPositionY() > 10000){
	...
	player.updateSpriteBasedOnHealth = function(){

		if(player.health > player.healthParts*2){
			player.character.stillAsset = orangeFlameStillAsset;
			player.character.movingAsset = orangeFlameMovingAsset;
		}else if(player.health > player.healthParts){
			player.character.stillAsset = redOrangeFlameStillAsset;
			player.character.movingAsset = redOrangeFlameMovingAsset;
		}else{
			player.character.stillAsset = redFlameStillAsset;
			player.character.movingAsset = redFlameMovingAsset;
		}
	}

	player.checkIfDamaged = function(){
		if(player.hp > 0 && player.invulnerableTime == 0){
			var c = 0;
			var highestDamage = 0;
	...
	player.wetnessControl = function(){
		...
		if(player.health > 0 && player.wetnessGraphic != undefined){
