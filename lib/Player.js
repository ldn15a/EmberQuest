var Player = function(x,y,asset){
	...
	player.update = function(){
		...
		keyboard.debug.togglePrintingOfAllReturns();
	...
	player.updateHealthGraphics = function(){
		...
		var yOffset = 6;
		var c = player.health;
		var bigGraphics = player.health % player.healthParts;

		for(var i = 0; i < bigGraphics; i++){
			player.healthGraphicArray[player.healthGraphicArray.length] = new HealthGraphic(xOffset,yOffset,1);
			c-=player.healthParts;
			xOffset += 10;
		}

		for(var i = 0; i < c; i++){
			player.healthGraphicArray[player.healthGraphicArray.length] = new HealthGraphic(xOffset,yOffset,0);
			yOffset += 2;
		}
	}
