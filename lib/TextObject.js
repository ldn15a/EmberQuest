...
function TextObject (string) {
    ...
    textObject.options = [];
    textObject.optionsOutcome = [];

    //add string to array
    textObject.addOption = function(string,goto){
        textObject.options.push(string);
        textObject.optionsOutcome.push(goto);
    ...
    textObject.removeOption = function(string){
        ...
        while(c < textObject.options.length){
            if(textObject.options[c] == string){
                ...
                textObject.optionsOutcome.splice(c,1);
                break;
            }
            c++;
        }
