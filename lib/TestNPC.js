function TestNPC (x,y,asset) {
        ...
        if(testNPC.active){
            ...
            if(!textBox.isShowing){
                ...
                if(testNPC.tOA.length == 0){
                    ...
                    if(textBox.selectedOption == testNPC.text[3]){
    ...
    testNPC.initialize = function () {
        ...
        testNPC.textLoaded = false;
        testNPC.text = [];

        fileReader.requestFile ("lib/NPCText.txt");
        if (fileReader.isFileReady ("lib/NPCText.txt")) {
            testNPC.text = fileReader.read ("lib/NPCText.txt");
            ...
            var needsOptions = TextObject(testNPC.text[2]);

            //you can add as many options as you'd like. The strings are supplied by our testNPC.text array.
            needsOptions.addOption(testNPC.text[3]);
            needsOptions.addOption(testNPC.text[4]);
            needsOptions.addOption(testNPC.text[5]);
            needsOptions.addOption(testNPC.text[6]);
            needsOptions.addOption(testNPC.text[7]);
            ...
            testNPC.query1 = TextObject(testNPC.text[16]);
            testNPC.query1.addOption(testNPC.text[17]);
            testNPC.query1.addOption(testNPC.text[18]);
            testNPC.query1.addOption(testNPC.text[19]);
            testNPC.query1.addOption(testNPC.text[11]);
        }
    }();
