function MainMenu () {
	...
	mm.update = function () {
		...
		if (keyboard.getKeyStatus ("e") == "pressed") {
            ...
            if (mm.private.selectedEntry == 0 && mm.private.selector.graphic.position.y != 480) {
                mm.private.selector.graphic.position.set (415, 480);
                audio.forcePlay ("menuSwitch");
            }
            //  Quit is selected
            else if (mm.private.selectedEntry == 1 && mm.private.selector.graphic.position.y != 575) {
	...
	mm.private.startGame = function () {
        ...
        audio.stop ("mainMenuBackground");
		areaController.loadArea ("lightMarsh");
        state = play;
	}
