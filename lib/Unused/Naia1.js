function Naia1 (x,y,asset) {
    ...
    naia1.update = function () {
    	...
    	if(naia1.ready){
	        if(naia1.active == false && naia1.character.hitTestRectangle(naia1.character, playerArray[0].character)){
	            ...
	            if(playerArray[0].canControl && naia1.option != 0){
        ...
        //gate
        if(naia1.gateOpen){
        	if(naia1.gateArray.length > 0){
        		if(naia1.active){
			        naia1.activeStorage = true;
			        naia1.active = false;
        		}
        		naia1.gateDemolitionTimer++;
        		if(naia1.gateDemolitionTimer > 10){
        			naia1.gateDemolitionTimer = 0;
        			var toDestroy = naia1.gateArray.shift();
        			var c = 0;
        		...
        		if(naia1.activeStorage){
        			naia1.activeStorage = false;
        			naia1.active = true;
        		}
        	}
        }else{
        	if(naia1.gateArray.length < naia1.maxWalls){
        		if(naia1.active){
			        naia1.activeStorage = true;
			        naia1.active = false;
        		}
        		...
        		if(naia1.gateDemolitionTimer > 10){
        			naia1.gateDemolitionTimer = 0;
        			naia1.gateArray.push(Wall(naia1.character.graphic.x+64,naia1.character.graphic.y-64*(naia1.maxWalls - naia1.gateArray.length-1),wallAsset));
	    			naia1.gateArray[naia1.gateArray.length-1].load();
        		}
        	}else{
        		if(naia1.activeStorage){
        			naia1.activeStorage = false;
    ...
    naia1.load = function(){
        ...
        for(var i = 0; i < naia1.maxWalls; i++){
	    	naia1.gateArray[i] = Wall(naia1.character.graphic.x+64,naia1.character.graphic.y-64*(i),wallAsset);
	    	naia1.gateArray[i].load();
		}
    }
