function DialogueParser (textFile, master) {
	...
	dialogueParser.hideOption;

	dialogueParser.assignText = function () {
		//console.log(dialogueParser.textFile);
        dialogueParser.text = fileReader.readFile(dialogueParser.textFile);
	...
	dialogueParser.parseCommand = function (string, runCommands) {
		...
		if(string.toLowerCase() == "end"){
		...
		}else if(string.toLowerCase() == "hide"){
			//console.log("HIDE FOUND.");
			isCommand = true;
			//ONLY BEFORE IT IS DISPLAYED.
			if (!runCommands) {
				dialogueParser.hideOption = true;
			}
			...
			for (var i = 3; i < string.length; i++) {
				...
				if (commandSearchMode < 3 && char === " " || char === ",") {
					...
					outcome1 = string.substring(startingIndex,string.length);
					//console.log(outcome1 + "| unique outcome - if and no else.");
				}
			}
			...
			if(justOneArgument){
				...
				if(checkPart2 === "=="){
					...
					if(isVariable){
						if(dialogueParser.checkCondition(checkPart1) != dialogueParser.checkCondition(checkPart3)){
							isCommand = dialogueParser.parseCommand(outcome1,runCommands);
							//console.log("Check completed. " + checkPart1 + " is NOT equal to " + checkPart3 + ". isVariable: "+isVariable+". Beginning outcome: "+outcome1);
						}else if(hasElse){
							isCommand = dialogueParser.parseCommand(outcome2,runCommands);
							//console.log("Check completed. " + checkPart1 + " is equal to " + checkPart3 + ". isVariable: "+isVariable+". Beginning outcome: "+outcome2);
						}else{
							isCommand = true;
							//console.log("Check completed. " + checkPart1 + " is equal to " + checkPart3 + ". isVariable: "+isVariable+". Beginning outcome: "+outcome2);
						}
	...
	dialogueParser.checkCondition = function (string) {
		...
		if (string === "renzelJob") {
				val = npcVariables.MerchantAgent_renzelJob;
			
		}else if (string === "askedTrade") {
				val = npcVariables.MerchantAgent_askedTrade;
			
		}else if (string === "askedVillage") {
				val = npcVariables.Naia_naiaFriendship;
			
		}else if (string === "askedWho") {
				val = npcVariables.MerchantAgent_askedWho;
			
		}else if (string === "blackIceRumorKnown") {
				val = npcVariables.Worried_blackIceRumorKnown;
		
		}else if (string === "firstEncounterWorried") {
				val = npcVariables.Worried_firstEncounterWorried;
			
		}else if (string === "naiaDistract") {
				val = npcVariables.Naia_naiaDistract;
			
		}else if (string === "wraithQuestion") {
				val = npcVariables.Naia_wraithQuestion;
			
		}else if (string === "naiaSuspicious") {
				val = npcVariables.Naia_naiaSuspicious;
			
		}else if (string === "wraithSoothed") {
				val = npcVariables.Naia_wraithQuestion;
			
		}else if (string === "naiaPlan") {
				val = npcVariables.Naia_naiaPlan;
			
		}else if (string === "thiefStory") {
				val = npcVariables.Naia_thiefStory;
			
		}else if (string === "askedFestival") {
				val = npcVariables.Naia_askedFestival;
			
		}else if (string === "motiveReveal") {
				val = npcVariables.Naia_motiveReveal;
			
		}else if (string === "numAsked") {
				val = npcVariables.Naia_numAsked;
			
		}else if (string === "childQuestion") {
				val = npcVariables.Naia_childQuestion;
			
		}else if (string === "tideQuestion") {
				val = npcVariables.Naia_tideQuestion;
			
		}else if (string === "elderQuestion") {
				val = npcVariables.Naia_elderQuestion;
			
		}else if (string === "naiaEnd") {
				val = npcVariables.Naia_naiaEnd;
			
		}else if (string === "naiaFriendship") {
				val = npcVariables.Naia_naiaFriendship;
			
		}if(string == "health"){
			val = playerArray[0].health;
		}else if (string === "karma") {
			val = karma;
		} else if (string === "toldTruth") {
			val = npcVariables.Naia_toldTruth;
		} else if (string === "enteredVillage") {
			val = npcVariables.Naia_enteredVillage;
		} else if (string === "expectedCaravan") {
			val = npcVariables.Naia_expectedCaravan;
		} else if (string === "naiaPartnered") {
			val = npcVariables.Naia_naiaPartnered;
		} else if (string === "naiasCharm") {
			//global variable.
			val = naiasCharm;
		} else if (string === "villageKnowledge") {
			val = npcVariables.Naia_villageKnowledge;
		} else if (string === "marshDangerKnown") {
			val = npcVariables.Naia_marshDangerKnown;
		} else {
			val = undefined;
	...
	dialogueParser.setCondition = function (string, value, runCommands) {
		...
		if (string === "renzelJob") {
			if (runCommands) {
				npcVariables.MerchantAgent_renzelJob = value;
			}
		}else if (string === "askedTrade") {
			if (runCommands) {
				npcVariables.MerchantAgent_askedTrade = value;
			}
		}else if (string === "askedVillage") {
			if (runCommands) {
				npcVariables.Naia_naiaFriendship = value;
			}
		}else if (string === "askedWho") {
			if (runCommands) {
				npcVariables.MerchantAgent_askedWho = value;
			}
		}else if (string === "blackIceRumorKnown") {
			if (runCommands) {
				npcVariables.Worried_blackIceRumorKnown = value;
			}
		}else if (string === "firstEncounterWorried") {
			if (runCommands) {
				npcVariables.Worried_firstEncounterWorried = value;
			}
		}else if (string === "naiaDistract") {
			if (runCommands) {
				npcVariables.Naia_naiaDistract = value;
			}
		}else if (string === "wraithQuestion") {
			if (runCommands) {
				npcVariables.Naia_wraithQuestion = value;
			}
		}else if (string === "naiaSuspicious") {
			if (runCommands) {
				npcVariables.Naia_naiaSuspicious = value;
			}
		}else if (string === "wraithSoothed") {
			if (runCommands) {
				npcVariables.Naia_wraithQuestion = value;
			...
			if (runCommands) {
				npcVariables.Naia_naiaPlan = value;
			}
		}else if (string === "thiefStory") {
			if (runCommands) {
				npcVariables.Naia_thiefStory = value;
			}
		}else if (string === "askedFestival") {
			if (runCommands) {
				npcVariables.Naia_askedFestival = value;
			}
		}else if (string === "motiveReveal") {
			if (runCommands) {
				npcVariables.Naia_motiveReveal = value;
			}
		}else if (string === "numAsked") {
			if (runCommands) {
				npcVariables.Naia_numAsked = value;
			}
		}else if (string === "childQuestion") {
			if (runCommands) {
				npcVariables.Naia_childQuestion = value;
			}
		}else if (string === "tideQuestion") {
			if (runCommands) {
				npcVariables.Naia_tideQuestion = value;
			}
		}else if (string === "elderQuestion") {
			if (runCommands) {
				npcVariables.Naia_elderQuestion = value;
			}
		}else if (string === "naiaEnd") {
			if (runCommands) {
				npcVariables.Naia_naiaEnd = value;
			}
		}else if (string === "naiaFriendship") {
			if (runCommands) {
				npcVariables.Naia_naiaFriendship = value;
			...
			if (runCommands) {
				karma = value;
				console.warn("Tried to set karma through dialogue.");
			}
		} else if (string == "health") {
			if (runCommands) {
				playerArray[0].health = value;
			}
		} else if (string == "IngisCard") {
			if (runCommands) {
				playerArray[0].inventory.pickup("IngisCard");
			}
		} else if (string == "flashPowder") {
			if (runCommands) {
				playerArray[0].inventory.pickup("flashPowder");
			}
		} else if (string === "toldTruth") {
			if (runCommands) {
				npcVariables.Naia_toldTruth = value;
			}
		} else if (string === "enteredVillage") {
			if (runCommands) {
				npcVariables.Naia_enteredVillage = value;
			}
		} else if (string === "expectedCaravan") {
			if (runCommands) {
				npcVariables.Naia_expectedCaravan = value;
			}
		} else if (string === "naiaPartnered") {
			if (runCommands) {
				npcVariables.Naia_naiaPartnered = value;
			}
		} else if (string === "naiasCharm") {
			if (runCommands) {
				//global variable.
			...
			if (runCommands) {
				npcVariables.Naia_villageKnowledge = value;
			...
			if (runCommands) {
				npcVariables.Naia_marshDangerKnown = value;
			}
		} else {
			isCommand = false;
			console.warn("Error: tried to set condition of variable \"" + string + "\" with value \"" + value + "\" from file \"" + dialogueParser.textFile + "\". Maybe you mispelled it? Check dialogueParser.js's setCondition function.");
		}
	...
	dialogueParser.parseSetup = function () {
		...
		while (c < dialogueParser.text.length) {
			...
			if (dialogueParser.text[c].substring(0, 3) === "if(") {
				...
				//the offset for the substring seems to be dependent on the line endings for some reason.
				//console.log(dialogueParser.text[c].substring(3, dialogueParser.text[c].length - 2));
				if (dialogueParser.parseConditions(dialogueParser.text[c].substring(3, dialogueParser.text[c].length - 2))) {
				...
				if (dialogueParser.parseConditions(dialogueParser.text[c].substring(8, dialogueParser.text[c].length - 2))) {
					//console.log("this is the starting index.");
					dialogueParser.relaventStartingIndex = c;
					break;
				}
	...
	dialogueParser.prepareNextTextObject = function () {
		...
		if (dialogueParser.ready) {
			...
			if (dialogueParser.goTo != null) {
				var c = 0;
				while (c < dialogueParser.relaventDialogue.length) {

					if (!isNaN(dialogueParser.relaventDialogue[c].substring(0, 1))) {
						var j = 0;
						var char;

						//what if the number is multiple digits long?
						while (true) {
				...
				//console.log("TESTSTRING: " + testString + "C: "+c);
				...
				displayText = dialogueParser.parseLine(dialogueParser.relaventDialogue[currentIndex], true);
			...
			//to ensure that our hideOptions didn't bleed over
			dialogueParser.hideOption = false;
			...
			while (true) {
				...
				var testParse = dialogueParser.parseLine(dialogueParser.relaventDialogue[c], false);
				//if we need to hide the option, don't add it.
				if(!dialogueParser.hideOption){
					textObject.addOption(dialogueParser.relaventDialogue[c], testParse);
