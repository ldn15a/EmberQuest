    ...
    naia2.update = function () {
    	...
    	if(naia2.ready){
	        if(naia2.active == false && naia2.character.hitTestRectangle(naia2.character, playerArray[0].character)){
	            ...
	            if(playerArray[0].canControl && naia2.option != 0){
	                ...
	                if(naia2.tOA.length == 0){
	                	...
	                	if(naiaBase.toldTruth){
	                		...
	                		if(naia2.option == 1){
		                		...
		                		var needsOptions = TextObject(naia2.text[55]);
								needsOptions.addOption(naia2.text[56],12);
								needsOptions.addOption(naia2.text[57],6);
								naia2.tOA.push(needsOptions);
		                	}else if(naia2.option == 3){
		                		var needsOptions = TextObject(naia2.text[58]);
								needsOptions.addOption(naia2.text[59],2);
								needsOptions.addOption(naia2.text[60],7);
								needsOptions.addOption(naia2.text[61],9);
								naia2.tOA.push(needsOptions);
		                	}else if(naia2.option == 4){
		                		var needsOptions = TextObject(naia2.text[62]);
								needsOptions.addOption(naia2.text[63],12);
								needsOptions.addOption(naia2.text[64],6);
								naia2.tOA.push(needsOptions);
		                	}else if(naia2.option == 5){
		                		naiaBase.naiaPartnered = false;
								naia2.tOA.push(TextObject(naia2.text[65]));
		                	}else if(naia2.option == 6){
		                		naiaBase.naiaPartnered = false;
								naia2.tOA.push(TextObject(naia2.text[66]));
		                	}else if(naia2.option == 7){
		                		var needsOptions = TextObject(naia2.text[67]);
								needsOptions.addOption(naia2.text[68],5);
								needsOptions.addOption(naia2.text[69],8);
								naia2.tOA.push(needsOptions);
		                	}else if(naia2.option == 8){
		                		var needsOptions = TextObject(naia2.text[70]);
								needsOptions.addOption(naia2.text[71],12);
								needsOptions.addOption(naia2.text[72],6);
								naia2.tOA.push(needsOptions);
		                	}else if(naia2.option == 9){
		                		var needsOptions = TextObject(naia2.text[73]);
								needsOptions.addOption(naia2.text[74],12);
								needsOptions.addOption(naia2.text[75],6);
								if(karma <= 0){
									needsOptions.addOption(naia2.text[76],10);
								}else{
									needsOptions.addOption(naia2.text[76],11);
								}
								naia2.tOA.push(needsOptions);
		                	}else if(naia2.option == 10){
		                		naiaBase.naiaPartnered = false;
								naia2.tOA.push(TextObject(naia2.text[77]));
		                	}else if(naia2.option == 11){
		                		naiaBase.naiasCharm = true;
								naia2.tOA.push(TextObject(naia2.text[78]));
								naia2.tOA.push(TextObject(naia2.text[79]));
		                	}else if(naia2.option == 12){
		                		naiaBase.naiaPartnered = true;
		                		naia2.endConversation();
		                	}else{
		                		naia2.endConversation();
