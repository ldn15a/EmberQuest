var Character = function(x,y,asset){
	...
	character.hitsPlatform = function(offset){
		var hit = false;
		if(offset > 0){
			for(var i = 0;i < wallArray.length;i++){
				if(wallArray[i].isPlatform){
	...
	character.hitsWallY = function(offset){
		...
		if(offset != 0){
			//we aren't hitting the platform yet (we're above it)
			platformHit = !character.hitsPlatform(offset);
			character.mY(offset);
			//we are hitting the platform now (we now know we're standing on it - so let's allow you to stand)
			platformHit = platformHit && character.hitsPlatform(offset);
			//override if you're holding down
			if(keyboard.getKeyHardwareStatus("s")){
				platformHit = false;
			}
		}
		var hit = character.hitsWall() || platformHit;
		if(offset != 0){
			character.mY(-offset);
		}
		return hit;
