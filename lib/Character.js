var Character = function (x, y, asset) {
	...
	var character = {};
	character.asset;
	character.stillAsset;
	character.movingAsset;
	character.graphic;
	...
	character.setSprite = function (asset) {
		...
		if(character.asset.name == "flameRed"){
			character.stillAsset = redFlameStillAsset;
			character.movingAsset = redFlameMovingAsset;
		}else if(character.asset.name == "flameOrangeRed"){
			character.stillAsset = redOrangeFlameStillAsset;
			character.movingAsset = redOrangeFlameMovingAsset;
		}else if(character.asset.name == "flameOrange"){
			character.stillAsset = orangeFlameStillAsset;
			character.movingAsset = orangeFlameMovingAsset;
		}else if(character.asset.name == "npc_flame"){
			character.stillAsset = npc_flame;
			character.movingAsset = npc_flame_right;
		}else if(character.asset.name == "naia_flame"){
			character.stillAsset = naia_flame;
			character.movingAsset = naia_flame_right;
		}else if(character.asset.name == "npc_flame_2"){
			character.stillAsset = npc_flame_2;
			character.movingAsset = npc_flame_2_right;
		}
	...
	character.calculatePositionX = function(xPos){
		return globalX - xPos - globalX + app.view.width/2;
	}
	...
	character.hitsWall = function(speed){
				...
				if(!cannotHitPlatform && speed >= 0){
					if(wallArray[i].character != undefined){
						if(character.hitTestRectangle(character, wallArray[i].character)){
							...
							if(!character.hitTestRectangle(character, wallArray[i].character)){
								hit = true;
								if(wallArray[i].wetness > highestWetness){
									highestWetness = wallArray[i].wetness;
								}
							}
							character.mY(speed);
						}
		...
		//determine how much water our character is currently taking on.

		character.wetness = highestWetness;
	...
	character.checkIfJumping = function(){
		if(!character.lastJumpInput && !character.jump && character.hitsWallY(1)){
			return true;
		}
		return false;
	}
	...
	character.movement = function (moveDirection, jumpInput) {
		...
		if(character.moveDir != 0){
			...
			if(character.lastDir != character.moveDir){
				character.setSprite(character.movingAsset);
				character.movingAsset.scaleX = character.movingAsset.defaultScaleX*character.moveDir;
			}
			...
			character.setSprite(character.stillAsset);
		}
