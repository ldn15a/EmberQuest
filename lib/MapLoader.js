var MapLoader = function () {
		...
		var generateMapObjects = function () {
			...
			for (var i = 0; i < lines.length; i++) {
                ...
                for (var j = 0; j < rowCells.length; j++) {
                    if (mapLoader.private.objIDMap.has(rowCells[j].trim())) {
                        if (rowCells [j].trim() == "999") {
                            console.error ("You tried to create a specific NPC that isn't Naia. Lucas will fix this later.");
                        }
                        else{
                            mapLoader.private.objIDMap.get(rowCells[j].trim())((j * 64) + 32, (i * 64) + 32);
                        }
                    }
                    else {
                        //alert("WARNING: Tried to load object from map with unknown id: " + rowCells[j]);
                        if (rowCells[j] != "\n") {
                            console.warn("WARNING: Tried to load object from map with unknown id: " + rowCells[j].trim());
                        }
                    }
                }
			}
    ...
    mapLoader.private.makeBrickDiamond = function (x, y) {
	...
	mapLoader.private.makeMarshMudWater = function (x, y) {
		mapLoader.private.currentAreaObjRef.private.grasses.push(Wall(x, y, marsh_mud_water_tile, false, 2));
	}

	mapLoader.private.makeMarshWaterMud = function (x, y) {
		mapLoader.private.currentAreaObjRef.private.grasses.push(Wall(x, y, marsh_water_mud_tile, false, 2));
	}

	mapLoader.private.makeMarshMudWaterMiddle = function (x, y) {
		mapLoader.private.currentAreaObjRef.private.grasses.push(Wall(x, y, marsh_mud_water_middle_tile, false, 4));
	}

	mapLoader.private.makeMarshWaterMudMiddle = function (x, y) {
		mapLoader.private.currentAreaObjRef.private.grasses.push(Wall(x, y, marsh_water_mud_middle_tile, false, 4));
	}

	mapLoader.private.makeMarshWater = function (x, y) {
		mapLoader.private.currentAreaObjRef.private.grasses.push(Wall(x, y, marsh_water_tile, false, 8));
    }
	...
	mapLoader.private.makeNaia = function (x, y) {
		mapLoader.private.currentAreaObjRef.private.NPCs.push(Naia(x, y, naia_flame));
	}

	mapLoader.private.makeGenericNPC = function (x, y) {
		mapLoader.private.currentAreaObjRef.private.NPCs.push(GenericNPC(x, y, npc_flame));
	}

	mapLoader.private.makeGenericNPCSpawner = function (x, y) {
		mapLoader.private.currentAreaObjRef.private.NPCs.push(GenericNPCSpawner(x, y, npc_flame));
    }
    ...
    mapLoader.private.makeBranchBase2 = function (x, y) {
        mapLoader.private.currentAreaObjRef.private.treeBranchBases.push (Wall (x, y, tree_branch_base2, true, false));
    }
    ...
    mapLoader.private.makeFlameStatue = function (x, y) {
        mapLoader.private.currentAreaObjRef.private.flameStatues.push (Wall (x, y, flame_statue, false, false));
    }

    mapLoader.private.makePrisonCell = function (x, y) {
        mapLoader.private.currentAreaObjRef.private.prisonCells.push (Wall (x, y, prison_cell, false, false));
    }

    mapLoader.private.makeWaterWraith = function (x, y) {
        mapLoader.private.currentAreaObjRef.private.waterWraiths.push (Enemy (x, y, water_wraith));
    }

    mapLoader.private.makeDoorwayGeneral = function (x, y) {
        mapLoader.private.currentAreaObjRef.private.doorwayGenerals.push (Wall (x, y, doorway_general, false, false));
    }
    
    mapLoader.private.makeWorriedNortherner = function (x, y) {
        mapLoader.private.currentAreaObjRef.private.NPCs.push (NPC( x, y, redFlameStillAsset, "WorriedNortherner"));
    }

    mapLoader.private.makeLoadingZoneMarshToVillage = function(x, y){
        mapLoader.private.currentAreaObjRef.private.loadingZones.push (LoadingZone( x, y, true, "village"));
    }

    mapLoader.private.makeLoadingZoneVillageToDeepMarsh = function(x, y){
        mapLoader.private.currentAreaObjRef.private.loadingZones.push (LoadingZone( x, y, true, "deepMarsh"));
    }

    mapLoader.private.makeLoadingZoneMarshToGrotto = function(x, y){
        mapLoader.private.currentAreaObjRef.private.loadingZones.push (LoadingZone( x, y, true, "Grotto"));
    }

    mapLoader.private.makeLoadingZoneGrottoToDeepMarsh = function(x, y){
        mapLoader.private.currentAreaObjRef.private.loadingZones.push (LoadingZone( x, y, true, "deepMarsh"));
    }

    mapLoader.private.makeLoadingZoneDeepMarshToBoss = function(x, y){
        mapLoader.private.currentAreaObjRef.private.loadingZones.push (LoadingZone( x, y, true, "bossRoom"));
    }

    mapLoader.private.makeMerchantCompanyAgent = function(x, y){
        mapLoader.private.currentAreaObjRef.private.loadingZones.push (NPC( x, y, npc_flame_2, "MerchantCompanyAgent1"));
    }
	...
	mapLoader.private.initialize = function () {
        ...
        */
        ...
        mapLoader.private.objIDMap.set("7", mapLoader.private.makeMarshMudGrass);
        ...
        mapLoader.private.objIDMap.set("8", mapLoader.private.makeMarshWaterMud);
		mapLoader.private.objIDMap.set("4", mapLoader.private.makeMarshWaterMudMiddle);
        mapLoader.private.objIDMap.set("6", mapLoader.private.makeMarshMudWaterMiddle);
        ...
        mapLoader.private.objIDMap.set("29", mapLoader.private.makeBrickDiamond);
		...
		mapLoader.private.objIDMap.set("40", mapLoader.private.makeWorriedNortherner);
		mapLoader.private.objIDMap.set("46", mapLoader.private.makeMerchantCompanyAgent);

		// Loading zones
		mapLoader.private.objIDMap.set("41", mapLoader.private.makeLoadingZoneMarshToVillage);
		mapLoader.private.objIDMap.set("42", mapLoader.private.makeLoadingZoneVillageToDeepMarsh);
		mapLoader.private.objIDMap.set("43", mapLoader.private.makeLoadingZoneMarshToGrotto);
		mapLoader.private.objIDMap.set("44", mapLoader.private.makeLoadingZoneGrottoToDeepMarsh);
		mapLoader.private.objIDMap.set("45", mapLoader.private.makeLoadingZoneDeepMarshToBoss);
        ...
        mapLoader.private.objIDMap.set("9", mapLoader.private.makeFlameStatue);
        mapLoader.private.objIDMap.set ("10", mapLoader.private.makePrisonCell);
        mapLoader.private.objIDMap.set ("21", mapLoader.private.makeDoorwayGeneral);

        //enemies
        mapLoader.private.objIDMap.set("15", mapLoader.private.makeWaterWraith);
        mapLoader.private.objIDMap.set("22", mapLoader.private.makeBranchBase2);
	}();
