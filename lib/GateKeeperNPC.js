function GateKeeperNPC (x,y,asset) {
	...
	gateKeeperNPC.fireParticleTimer;
    ...
    gateKeeperNPC.update = function () {
        ...
        if(gateKeeperNPC.gateOpen){
        	...
        	console.clear();
        	console.log(gateKeeperNPC.gateArray.length);
        }else{
        ...
        if(gateKeeperNPC.hasFireParticles){
	        ...
	        //fire particles
			gateKeeperNPC.fireParticleTimer++;
			if(gateKeeperNPC.fireParticleTimer > 3){
				gateKeeperNPC.fireParticleArray.push(FireParticle(gateKeeperNPC.character.graphic.x,gateKeeperNPC.character.graphic.y,gateKeeperNPC));
				gateKeeperNPC.fireParticleTimer = 0;
			}

			for(var i = 0;i < gateKeeperNPC.fireParticleArray.length;i++){
				gateKeeperNPC.fireParticleArray[i].update();
			}
		}
    ...
    gateKeeperNPC.scroll = function(){
        ...
        if(gateKeeperNPC.hasFireParticles){
	        for(var i = 0;i < gateKeeperNPC.fireParticleArray.length;i++){
				gateKeeperNPC.fireParticleArray[i].scroll();
			}
		}
    ...
    gateKeeperNPC.depth = function(){
    	if(gateKeeperNPC.hasFireParticles){
	    	for(var i = 0;i < gateKeeperNPC.fireParticleArray.length;i++){
				gateKeeperNPC.fireParticleArray[i].depth();
			}
		}
    ...
    gateKeeperNPC.unload = function(){
        gateKeeperNPC.destroyInteractGraphic();
        if(gateKeeperNPC.hasFireParticles){
	        while(gateKeeperNPC.fireParticleArray.length > 0){
				gateKeeperNPC.fireParticleArray[0].destroy();
			}
		}
