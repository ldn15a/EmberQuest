function NPC(x, y, asset, textFile) {
    ...
    npc.checkVariables = function(){
        ...
        //console.log("sl;dkfjas;ldkfj");
        if(npc.generic_zeldaWalk){
            //console.log("ZEEEELLLLLLLDAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA");
            isMoving = true;
            npc.NaiaZeldaWalk();
        }
    ...
    npc.NaiaZeldaWalk = function(){
        //console.log("test");
        if(npc.generic_zeldaTime < npc.generic_zeldaWalkRange && npc.character.getPositionX() < npc.character.calculatePositionX(npc.originX) + npc.generic_zeldaWalkRange && npc.character.getPositionX() > npc.character.calculatePositionX(npc.originX) - npc.generic_zeldaWalkRange){
            npc.character.movement(npc.generic_zeldaWalkDir*npc.generic_zeldaWalkSpeed,false);
            npc.generic_zeldaTime++;

        }else{
            npc.character.movement(0,false);
            if(npc.character.graphic.x < -32 || npc.character.graphic.x > app.view.width + 32){
                npc.unload();
    ...
    npc.initializeVariables = function () {
        ...
        npc.generic_zeldaWalkSpeed = 1;
        npc.generic_zeldaWalkRange = 900 + randomInt(0,5)*20;
    }
